...
import { type MovieDBMovieIdResponse } from '../../../infrastructure/interfaces/movie-db.responses';
...
export const movieDetailsUseCase = async (fetcher: HttpAdapter, movieId: number): Promise<MovieDetails> => {
  try {
    const details = await fetcher.get<MovieDBMovieIdResponse>(`/${movieId}`);
    return MovieMapper.fromMovieIdDBResultToEntity(details);
  } catch (error) {
    console.log(error);
